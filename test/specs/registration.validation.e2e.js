...
describe("Registration", () => {
  it("User does not enter any inputs on registration page", async () => {
    ...
    await expect($("#mat-error-0")).toHaveTextContaining(
      "Please provide an email address."
    );

    await expect($("#mat-error-1")).toBeExisting();
    await expect($("#mat-error-1")).toHaveTextContaining(
      "Please provide a password."
    );

    await expect($("#mat-error-2")).toBeExisting();
    await expect($("#mat-error-2")).toHaveTextContaining(
      "Please repeat your password."
    );

    await expect(
      $$("//mat-password-strength-info/mat-card/mat-card-content/div")
    ).toBeElementsArrayOfSize(5);

    await expect($("#mat-error-3")).toBeExisting();
    await expect($("#mat-error-3")).toHaveTextContaining(
      "Please select a security question."
    );

    await expect($("#mat-error-4")).toBeExisting();
    await expect($("#mat-error-4")).toHaveTextContaining(
